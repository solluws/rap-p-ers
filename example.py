...
in a community this script creates and removes.
    ...
    python example.py --token            # log in with $ROOT_TOKEN, run it all
...
EMAIL_PATTERN = os.environ.get("ROOTPY_EMAIL_PATTERN", "you-{tag}@example.com")
# Set ROOTPY_EMAIL_PATTERN to a domain whose mail you can actually read;
# Root sends a verification code to it.
...
# ===================================================================== #
def load_token() -> str:
    """The token comes from the environment; never hard-code one here."""
    token = os.environ.get("ROOT_TOKEN", "").strip()
    if token:
        return token
    raise SystemExit(
        "No token found. Set ROOT_TOKEN to your account token:\n"
        "    export ROOT_TOKEN='<your token>'   (macOS/Linux)\n"
        "    set ROOT_TOKEN=<your token>        (Windows)"
    )
...
async def client_from_new_account() -> RootClient:
    """Create an account, then use it.

    The step-by-step flow, and the rules that break it:
    See the website for the two-step signup walkthrough.
    """
...
async def discovery(client: RootClient, state: dict) -> None:
    """Community Discovery: the public directory, search and listing."""
    ...
    def named(enum_cls, value) -> str:
        """Never ``EnumClass(value)`` on a server value.

        ``CommunityCategory`` has gaps at 5, 8 and 9, and the server is free
        to add a member before rootpy learns about it, so the plain
        constructor raises ValueError on perfectly valid data. ``coerce``
        hands back the int instead.
        """
        member = enum_cls.coerce(value)
        return member.label if hasattr(member, "label") else f"<{member}>"

    results, total = await client.discovery.search(limit=5)
    step(f"discovery.search(limit=5) -> {len(results)} of {total} listed")
    for row in results[:3]:
        detail(f"{row.name} -- {row.member_count} members, "
               f"{named(CommunityCategory, row.category)}")

    ranked, _ = await client.discovery.search(
        category=CommunityCategory.GAMING,
        sort_by=CommunityDiscoverySort.MEMBER_COUNT,
        limit=3,
    )
    step(f"...filtered to GAMING, biggest first -> {len(ranked)}")

    # join() is shown, not run: it joins a real community owned by somebody
    # else, which is not a thing a guided tour should do on your behalf.
    detail("await client.discovery.join(row.id)")

    community_id = state.get("community_id")
    if community_id:
        listed = await client.community_service.get(community_id)
        status = named(
            CommunityDiscoverableRequirement, listed.is_discoverable_status)
        step(f"community_service.get() -> is_discoverable="
             f"{listed.is_discoverable}, first unmet requirement: {status}")
        detail("await client.community_service.set_discoverable(id, True)")
        detail("  -- owner only, and refused until every requirement is met")
...
def choose_login() -> str:
    ...
    print("  1. use an existing token ($ROOT_TOKEN)")
