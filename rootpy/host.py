...
class MultiClientHost:
    ...
    proxy:
        ...
        deliberate. It used to be accepted and half-applied --
        :class:`~rootpy.client.RootClient` ignores ``proxy`` when it is handed
        a transport, so the account's API calls went out direct while its
        gateway websocket, which reads ``client.proxy`` separately, went
        through the tunnel. Half-proxied is worse than either.
    shared_transport:
        **On by default.** One transport means one TLS + HTTP/2 handshake for
        ...
        This used to default to off because the transport's rate-limit
        cooldown table was shared along with the pool, so one account's 429
        paused every account. Cooldowns are keyed per account now (see
        ``GrpcWebTransport._account_scope``), which removed the reason.
        Pass ``shared_transport=False`` if you want hard socket isolation
        anyway.
    """
    ...
    def add(
        ...
        **client_options,
        ...
        if "proxy" in client_options:
            # Silently half-applied before this check: the host always hands
            # RootClient a transport, and RootClient ignores `proxy` when it
            # gets one -- so the API calls went out direct while the gateway,
    ...
    async def join(self, code: str, **broadcast_options) -> dict[str, Outcome]:
        ...
        Measured, because it is not what you would guess: Root answers
        ``UNAUTHENTICATED (16)`` for an account that is **already a member**,
        and the same for the community's own **owner**. Not ``ALREADY_EXISTS``,
        which is what this docstring claimed until
        ``TestBroadcastJoin`` ran it against a real invite twice over. A
