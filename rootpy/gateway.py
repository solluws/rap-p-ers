...
class Gateway:
    def __init__(
        ...
        *,
        ...
        # debugging aid: nothing in the library reads either key, and the
        # decoded ``fields`` (plus ``raw_hex``/``packet_raw_hex``) carry the
        # same information in the form callers actually use.
        #
        ...
        # on the event loop.
        #
        # Set True when you need to inspect an unfamiliar packet:
        ...
        try:
            while not self._stopping:
                ...
                if healthy:
                    ...
                    # reset the backoff ladder: a connection that delivered
                    # data proves the cursor is good, so the previous failures
                    # should not keep costing us.
                    attempt = 0
                    log.debug(
    ...
    def _notification_info(
        ...
        (the server omits field 1 when the code is
        ``PACKET_ERROR_CODE_UNSPECIFIED``).
        """
        sequence = None
        packet_case = None
    ...
    async def _keepalive(self, websocket) -> None:
        ...
        # then repeats on a short fuzzy interval, well inside the hub's idle
        # window. We lead with a fast first ping because each (re)connect
    ...
    def _connect_url(self) -> str:
        ...
        """
        # The cursor is the highest SequenceNumber of *any* received packet --
        # it updates from every packet, including pings -- and resume is
        # cursor + 1.
