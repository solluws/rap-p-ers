...
class CommunityManager:
    # Taken from rootpy.enums.ChannelType rather than redefined. Writing the
    # numbers by hand is a trap: 0 is UNSPECIFIED, which Root rejects with a
    # PredicateValidator on ChannelType, and a literal 1 means Text, so a
    # voice helper that sends 1 silently creates a text channel -- the worse
    # of the two failures, since it succeeds.
    ...
    async def list(self, *, refresh: bool = True) -> Tuple[Community, ...]:
        ...
        forcing expansion. Expanding means one ListMine plus a full
        ``CommunityGetExtended`` per community -- each returning that
        community's entire member, role and channel dump -- so an account in
        nine communities pays ten requests where one would do, and the return
        type here is ``Community``, not ``CommunityExtended``, so none of
        that data is needed to answer the call.

        This agrees with the login path, which also honours
        ``expand_communities``, so the documented cost model ("eager: 3 + one
        per community", opt-in) is true of both. Detail is still fetched and
        cached on first access by ``community_detail`` / ``get_community``.
        """
        if refresh:
    ...
    def held(self, community_id: str, *more: str) -> "_HeldCommunities":
        ...
        nothing raised -- within about 30 seconds with no gateway, and a
        little longer when the socket dies under it. This re-sends it every
        time the socket comes back, and opens one if there isn't one.
    ...
    async def edit_role(
        ...
        *,
        ...
        ``mentionable`` and ``self_assignable`` default to ``None`` for the
        same reason. They were ``False``, which the layer below could not tell
        apart from "not supplied", so it read the current value back and
        re-sent it: turning either flag *off* reported success and changed
        nothing. ``None`` means leave it alone; ``False`` now means off.
        """
