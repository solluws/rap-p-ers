...
knew -- and every one of these rules was established the hard way against the
live API.
...
def validate_nickname(value: str, *, field_name: str = "nickname") -> str:
    ...
    the evidence supports rather than proves. Confirmed live: ``rootpytester``,
    ``RootpyTester`` and ``rootpytester1`` are accepted; ``rootpy tester`` and
    ``rootpy-tester`` are rejected. That matches the username charset exactly
    -- letters, digits, underscores, periods and nothing else.
