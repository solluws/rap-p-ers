...
class PacketType(IntEnum):
    ...
    #: PACKET_TYPE_COMMUNITY_RELOAD = 5304. What a client is expected to do
    #: when one arrives is not documented anywhere, so rootpy decodes and
    #: dispatches it as ``packet_community_reload`` rather than dropping it
    #: silently; it gets no typed event for the same reason COMMUNITY_DELETED
    #: does not.
    COMMUNITY_RELOAD = 54
    ...
    #: The per-conversation notification setting for DMs
    #: (``DirectMessageGrpcService/SetNotificationSetting``). Slot 127, not
    #: 126 -- 126 is COMMUNITY_APP_SET_CHANNEL_ACTIVITY, which is out of
    #: order in the server's own oneof.
    DIRECT_MESSAGE_SET_NOTIFICATION_SETTING = 127
