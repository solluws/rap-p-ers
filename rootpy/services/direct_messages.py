...
#: Per-conversation notification setting. The packet
#: it produces is PacketContainer slot 127.
DIRECT_MESSAGE_SET_NOTIFICATION_SETTING = f"{BASE}/SetNotificationSetting"
...
class DirectMessageService:
    ...
    async def set_notification_setting(
        ...
        """Set your notification setting for one DM conversation.

        .. warning::
           **The server does not implement this yet.** Every call raises
           :class:`~rootpy.exceptions.GrpcUnimplemented` (status 12,
           ``Method is unimplemented``). Its community-side twin,
           :meth:`~rootpy.services.communities.CommunityService.set_notification_setting`,
           is undeployed in exactly the same way. The protocol declares both
           RPCs and both request messages; the deployed API has shipped
           neither. Both methods are here so they work the day the server
           does, and so the failure is a clear typed exception.

        The value is
        :class:`~rootpy.enums.NotificationStatus`. A successful call is
        expected to echo back to every one of your sessions as PacketContainer
        slot 127, ``DIRECT_MESSAGE_SET_NOTIFICATION_SETTING`` -- expected
        rather than observed, because no call has yet succeeded.
        """
        direct_message_id = normalize_root_guid(direct_message_id)

        high64, low64 = create_command_idempotency_guid()
        context = length_field(4, encode_root_guid_parts(high64, low64))

        payload = bytearray()
        payload += length_field(1, context)
        payload += length_field(10, encode_root_guid(direct_message_id))
        payload += field_key(11, 0) + encode_varint(int(notification_setting))

        await self._dm_unary(
            DIRECT_MESSAGE_SET_NOTIFICATION_SETTING,
            bytes(payload),
            "DirectMessageSetNotificationSetting",
            [],
        )
