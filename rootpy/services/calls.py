...
class CallService:
    ...
    def _parse_session(cls, body: bytes) -> dict[str, Any]:
        result: dict[str, Any] = {
            ...
            # WebRtcSessionCreateResponse fields 17/18/19
    ...
    async def create_session(
        ...
        *,
        ...
        if supports_v2:
            # WebRtcSessionCreateRequest field 13 (SupportsV2). Sent only when
            # true; an absent proto3 bool reads as false on the server either
            # way.
            #
            # This defaults to True because **the server refuses the v1 path
            # outright**: omitting field 13 answers FAILED_PRECONDITION (9)
            # for a DM container and for a community channel alike. Passing
            # supports_v2=False is still allowed so the old shape can be sent
            # deliberately, but it does not currently work.
        ...
        # A v2 session has NO session_id. Root does not negotiate the media
        # itself any more: it hands back a LiveKit server URL (field 18) and
        # a JWT (field 19) and expects the client to join that room. A v2
        # answer carries fields 6 (members), 7-10 (bandwidths), 17
        # (backend=2), 18 and 19 -- and neither 4 nor 5.
        #
        # Requiring session_id would therefore raise on a response that had
        # in fact succeeded. Insist on *some* usable route instead: a v1
        # session id, or a v2 url-and-token pair.
        has_v2_route = bool(parsed["server_url"] and parsed["access_token"])
        if not parsed["session_id"] and not has_v2_route:
            raise RuntimeError(
                "SessionCreate returned neither a session ID nor a media "
                "route (server_url + access_token)"
            )
...
"""
    ...
    async def play_audio(
        ...
        *,
        ...
        # The SDP is logged rather than written to disk or printed: a file in
        # the user's home directory fails on hosts with no Desktop folder
        # (containers, headless servers, most Linux setups), and printing
        # corrupts the stdout of any CLI or piped program that plays audio.
        # To see it::
        #
