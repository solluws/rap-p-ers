...
class AssetService:
    ...
    async def get(self, uris, *, chunk_size: int = 5,
        ...
        **Small chunks, wide concurrency -- the opposite of the usual advice.**
        ...
        costs about the same whatever the payload (see
        ...
        ``Uris: The specified condition was not met for 'Uris'``.

        So the bigger the chunk, the likelier it contains a poison URI, and
        the whole chunk then degrades to one-request-per-URI. The defaults
        sit at the knee of that curve: ``chunk_size=5`` keeps a poison URI
        cheap, and concurrency past 16 flattens out, so raising it only
        widens your exposure to rate limiting.
    ...
    # default here silently overrides the tuned one below.
    ...
    def uri_for_id(asset_id, kind: str = "image") -> str:
        ...
        file's, for one -- and every method here takes URIs, so this is how
