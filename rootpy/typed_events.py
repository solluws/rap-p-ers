...
class MemberEvent(RootEvent):
    ...
    COMMUNITY_MEMBER_DETACH), which say nothing about membership -- an attach
    is an online/offline signal, and ``CommunityMemberAttachPacket`` carries a
    ``UserOnlineStatus`` precisely because of that.
    """
...
class RoleEvent(RootEvent):
    ...
    # the role moved to the top of the list.
    before_role_id: str = ""
...
class ReactionEvent(RootEvent):
    ...
    channel reactions 5801/5802 and direct-message reactions 204/205, told
    apart by the packet's own ``packet_type``. Add and remove are the event
    name;
...
class TypedEventBuilder:
    ...
    #
    # ATTACH/DETACH are presence, not membership: CommunityMemberAttachPacket
    # carries a UserOnlineStatus precisely because it is an online/offline
    # signal. COMMUNITY_JOINED and
    ...
    # with it, not by the slot. Routing on the slot alone meant a rename,
    ...
    # 5801/5802 are the channel add/remove pair and 204/205 the
    # direct-message pair; all four arrive as the same packet message. Without
