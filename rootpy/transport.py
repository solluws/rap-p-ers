...
def _proxy_kwarg() -> str:
    ...
    routinely replaced -- test doubles and tracing wrappers subclass it with
    ``(*args, **kwargs)``, which advertises neither name, so a signature read
    on the replacement class can pick the wrong spelling.
...
class GrpcWebTransport:
    def __init__(
        ...
        *,
        ...
        #: will ever make it work -- it just burns the retries and the
        #: backoffs. Narrow this, or pass ``should_retry``, when you know
        #: which calls lie.
    ...
    def _account_scope(headers: Optional[dict]) -> str:
        ...
        the transport -- so without scoping, a transport shared between
        accounts would let one account's 429 hold up every other account on
        it. Sharing a transport is worth roughly 831 ms of TLS setup per
        account, so the per-account keying is what makes sharing free of that
        penalty.
