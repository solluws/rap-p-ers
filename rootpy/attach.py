...
How fast it decays, as seen by another account reading
``CommunityGetExtendedResponse.AttachedUserIds``:

* an attach made with **no gateway at all** is gone within about 30 seconds;
* one whose socket dies under it survives a little longer, under a minute.
...
or its own host-wide scheduler, because at a large account count the socket
handshakes have to be rationed across accounts in a way a single client
cannot see.
"""
