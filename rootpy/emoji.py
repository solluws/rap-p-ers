...
def normalize_reaction(value: str) -> str:
    ...
    if _emoji is None:
        raise ValueError(
            f"cannot resolve {value!r} to a Root shortcode. The 'emoji' "
            "package is not installed, so only a 20-entry fallback table of "
            "common emoji is available. It is a declared dependency of "
            "rootpy-client, so reinstall the package, or pass a shortcode "
            "such as :grin: instead."
        )
