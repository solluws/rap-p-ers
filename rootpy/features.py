...
def _container_key(container):
    """A ``RootUuid`` message -> a GUID string, or None when it is empty.

    Root nests ids as a two-field message (``{fixed64 high, fixed64 low}``),
    so the value handed back by the structured layer is an object, not a
    string. Stringifying it yields a repr, which is never a usable key.
    """
    ...
    raw = _field(container, "raw")
    if raw is None:
        raw = getattr(container, "raw", None)
    if isinstance(raw, (bytes, bytearray)):
        if not raw:
            return None
        from .protocol import decode_root_guid_message

        return decode_root_guid_message(bytes(raw))

    from .identifiers import format_root_guid

    high = _field(container, "high64")
    low = _field(container, "low64")
    if high is not None and low is not None:
        return format_root_guid(int(high), int(low))
    return None
...
def _notification_counterparty(notification):
    ...
    whoever sent it. The top-level id is the recipient's, not the sender's.
    The counterparty lives in the payload instead:
...
class FriendRequestManager:
    ...
    async def send(self, username: str):
        ...
        Root addresses friend requests by username, not id.

        A success here is **not** evidence the invite was delivered. And a
        second ``send`` to the same person can raise ``GrpcAlreadyExists``
        (6) even though neither of you sees a pending request: a *declined*
        invite is not deleted, it only moves out of :meth:`pending` and into
        :meth:`responded`, and it blocks a fresh invite in **both**
        directions until it is cleared. ``client.friends.remove(user_id)``
        clears it.
        """
...
class NotificationManager:
    ...
    async def counts_by_container(self) -> dict:
        ...
        The key is a GUID string, or ``None`` for the account-wide bucket --
        notifications that belong to no community and no conversation, which
        is where friend requests land. Root sends an empty ``RootUuid`` for
        that bucket rather than omitting the field.

        ``container_id`` is a nested ``RootUuid`` message, not a string. This
        used to key the dict on ``str()`` of that object, which produced
        ``"{'raw': b''}"`` -- a key that matches no community id, no
        conversation id and nothing a caller could construct. The total was
        right, so :meth:`count_unviewed` looked correct and the breakdown was
        unusable; a test asserted the mapping was a ``dict`` and that its
        values summed, and neither of those notices a broken key.
        """
        ...
        for entry in entries:
            ...
            counts[_container_key(container)] = int(value or 0)
        return counts
...
class UserSettingsManager:
    ...
    # address".
    #
    # The wire field is ``IsEmailVerified``; there is no ``is_required`` field
    # on any of the three requests, and the codec raises ``TypeError`` before
    # the request is built if you pass one.
