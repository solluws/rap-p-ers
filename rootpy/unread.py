...
That is ``root.CommunityGrpcService/Attach``. Before an attach, nothing at
all arrives for that community; after one, a channel message shows up in
about 0.4 s. This reader attaches to every community you are in, so you do
not have to.
...
What arrives once you are attached:
...
The unread test itself is Root's own:

    last_activity_at > user_last_viewed_at, both non-null, and not a voice
    channel

The "both non-null" half matters and is easy to get wrong: a channel you have
never opened has no ``user_last_viewed_at`` and is **not** unread. Treating
null as unread makes every channel in every new community look unread forever.
"""
...
class UnreadReader:
    ...
    You can still drive a pass by hand::
