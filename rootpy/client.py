...
class RootClient(HighLevelMixin):
    ...
    def __init__(
        ...
        *,
        ...
        #: Community Discovery. Its own service rather
        #: than methods on ``community`` because the server models it that
        #: way: ``root.CommunityDiscoveryGrpcService``, and a directory row is
        #: not a ``Community``.
        self.discovery = CommunityDiscoveryService(
            self.transport,
            self._require_token,
        )
    ...
    async def connect(self, *, announce_device: bool = True) -> None:
        ...
        default is what a connection Root will deliver to looks like.
        """
    ...
    async def _announce_device_online(self) -> None:
        """Tell Root this device is Active.

        Root expects a device status to be announced on every connection, and
        repeated after each reconnect. A connection that never comes online is
        not one Root has any reason to deliver to.

        It matters in practice: a connection that has not announced a device
        status does not receive DM deliveries, while an otherwise identical
        one that has receives them within about half a second.
