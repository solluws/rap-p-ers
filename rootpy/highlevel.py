...
class HighLevelMixin:
    ...
    async def get_members_detailed(
        ...
        *,
        ...
        Both defaults are sized to what Root actually does, and both are
        worth tuning on a large community:

        * ``batch_size``. A profile request costs about 190 ms whatever the
          batch size, and Root accepts up to 4,000 ids in one call. Per-id
          cost falls from 1.96 ms at 100 to 0.5 ms at 1,000 and flattens
          after that, so 500 is the knee: most of the saving, and a failed
          batch still only costs 500 profiles rather than thousands.
        * ``concurrency``. The batches share one HTTP/2 connection, which
          multiplexes them, so issuing 8 at once costs barely more than
          issuing one -- the difference between a serial sweep and a
          concurrent one is the larger of the two effects on a big community.

        Set ``concurrency=1`` to issue the batches strictly in order. A batch
        that fails is logged and skipped -- those members come back with
    ...
    async def list_messages(
        ...
        *,
        ...
        ``limit`` defaults to None deliberately: omit it and the server
        applies its own page size of 50, which is larger than anything you
        can ask for.
    ...
    def watch_unread(
        ...
        **Prefer :class:`rootpy.unread.UnreadReader` for new code.** This
        method polls. Channel messages do push, but only after
