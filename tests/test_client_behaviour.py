...
def test_sweep_surfaces_first_new_message():
    ...
    async def run():
        ...
        # wall clock right up until the machine is busy -- this flaked once in
        # three full offline runs, and a flaky gate before a live run is worse
        # than a slow one. Absence still gets a fixed sleep above, because
        # polling for "nothing happened" is not a thing.
