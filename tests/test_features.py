...
def test_unavailable_can_be_made_non_retryable():
    ...
    Retrying it cannot ever help: an account that is not a member of the
    community will not become one between attempts, so the default retry on
    14 costs two round trips and two backoffs and then fails anyway.
    """
    from rootpy.transport import GrpcWebTransport
...
def test_members_detailed_default_batch_is_500_not_100():
    """Pin the default, because lowering it is silently expensive.

    A round trip costs about the same whether it carries 1 id or 200, and
    Root accepts up to 4,000 in one call, so a small batch buys nothing and
    costs a request each time. 2,000 members should be 4 requests, not 20.
    """
    async def run():
...
def test_no_undefined_lazy_names():
    ...
    package = pathlib.Path(__file__).resolve().parents[1] / "rootpy"
...
def test_asset_decode_matches_live_response_shape():
    """An AssetGet response carries BOTH the legacy map (field 1, AssetImage)
    and the assets map (field 2, AssetInformation). Mishandling the legacy
    entry makes the whole call return nothing."""
    async def run():
        from rootpy.protocol import grpc_frame
    ...
    async def run():
        ...
        try:
            factory = AccountFactory(
                email_pattern="hello-{tag}@example.com", username_prefix="test",
            )
...
def test_account_factory_rejects_a_pattern_without_tag():
    ...
    try:
        AccountFactory(email_pattern="hello@example.com")
    except ValueError:
        return
...
def test_saved_accounts_can_be_redacted(tmp_path):
    ...
    factory.accounts.append(
        CreatedAccount(
            ...
            email="hello-abc@example.com", token="secret-token",
        )
    )
    ...
    assert "hello-abc@example.com" in text
...
def test_signup_uses_connect_service_field_numbers():
    ...
    async def run():
        ...
        try:
            await AuthClient.signup(
                auth, "someuser", "password", "hello-abc@example.com",
    ...
    assert fields[6] == b"hello-abc@example.com"
...
def test_two_step_signup_reuses_every_detail():
    ...
    assert challenge.email.endswith("@example.com")
...
def test_account_factory_routes_signup_through_its_proxy():
    ...
    async def run():
        ...
        try:
            factory = AccountFactory(
                email_pattern="service-{tag}@example.com",
