"""Coverage for surfaces that had no tests at all.

The original 109 tests were concentrated in wire/protocol/cache. Seventeen
modules -- including ``highlevel``, ``structured_api``, ``features``,
``object_api``, ``domain_managers``, ``commands``, ``pagination``,
``permissions`` and ``typed_events`` -- had zero references, and 89 public
methods were never exercised by any test, example or devscript.

This file covers everything in that set that can be checked without a network
connection: permission algebra, pagination, stats, enum helpers, model
properties, the command context's mention parsing, the registry loader, and
the shape of the high-level surface. Anything that genuinely needs a server
lives in ``test_live_integration.py``.
"""
...
class TestListCommunitiesHonoursTheLazyDefault:
    ...
    dump. Measured against a real account in 9 communities, one of them with
    31,555 members: 10 requests and 1.73 s, where 1 request and 0.19 s do.

    Nothing needed that data — ``list`` returns ``Community``, not
    ``CommunityExtended``. It only warmed a cache the caller may never read,
    and it made the cost table in README.md and LLMS.md ("eager: 3 + one per
    community", opt-in) false for this path.
    """
...
class TestAssetBatchingStaysTuned:
    ...
    These defaults are measured, not chosen: AssetGet inverts the SDK's usual
    "bigger batches win" rule, because one URI Root refuses rejects the whole
    request and ~6% of real profile asset URIs are refused. Over 200 real URIs,
    chunk=100 took 18.56 s and chunk=5 took 1.63 s.
    ...
    surface it, because both spellings still *work*, just slower. That is the
    shape `defaultcheck.py` was written for, and it does not catch this pair
    (both defaults are truthy and merely differ), so it is pinned here.
    """
    ...
    def test_the_chunk_stays_small(self):
        ...
        assert chunk <= 10, (
            ...
            "request. Measured: chunk=100 was 11x slower than chunk=5."
        )
...
class TestTheLibraryWritesNothingUninvited:
    ...
    def _library_sources(self):
        return [
            ...
            and "devscripts" not in path.parts
        ]
...
class TestEnumsAndPackets:
    ...
    def test_active_status_is_0x10(self):
        """Regression: the generator skipped this hex literal (see HANDOFF)."""
...
class TestRegistryLoading:
    ...
    def test_short_names_are_unique_in_this_dataset(self):
        ...
        HANDOFF warns that connect.* and root.* reuse message *shapes* across
        services, but in the current descriptors no short name is ambiguous.
        The value stays a list so the disambiguation path keeps working if a
        future dump does collide -- this test will fail loudly if that happens.
        """
        ambiguous = {k: v for k, v in SIMPLE_MESSAGES.items() if len(v) > 1}
        assert ambiguous == {}

    def test_turnstile_token_is_field_three(self):
        """Regression for the field-3-not-7 finding in LLMS.md."""
    ...
    def test_services_and_enums_load(self):
        assert len(SERVICES) == 31 and len(RPC_SERVICES) == 31
        # 86 since client 0.9.128 added WebRtcBackend.
        assert len(ENUMS) == 86
...
class TestImportHygiene:
    ...
    def test_missing_media_raises_actionable_error(self):
        ...
        assert "rootpy[voice]" in str(exc)
...
# --------------------------------------------------------------------------
# contract for the live suite -- catches wrong-layer mistakes without a token
# --------------------------------------------------------------------------
class TestLiveSuiteContract:
    """Assert the attributes and shapes the live tests depend on.

    The first live run failed because the tests were written against
    ``CommunityAdminService`` (``create_community``) while ``client.community``
    is actually ``CommunityManager`` (``create``), and because
    ``is_connected`` is a property that was called as a method. There are 31
    manager/service attributes on the client, so picking the wrong one is easy.
    These checks need no network and fail in CI instead of mid-run.
    """
...
class TestCommunityCreateDefaults:
    ...
    field. ``example.py`` hit this too. Caught by the first live run.
    """
...
class TestChannelTypeConstants:
    ...
    def test_create_channel_does_not_default_to_unspecified(
        ...
        server, while README.md:334 and docs/api.md:102 both show callers
        passing 1 explicitly and never say the default is broken.
        """
        import importlib
...
# --------------------------------------------------------------------------
# contract: every client call in the live suite must match its sync/async-ness
# --------------------------------------------------------------------------
def _client_calls_in_live_suite():
    ...
    A call inside a ``lambda:`` handed to ``eventually()`` or ``ladder()`` is
    a third case, and neither failure mode this guard exists for applies to
    it. Both helpers call the thunk and then ``await`` the result only if it
    is awaitable, so a sync method is as correct there as an async one. Those
    ...
    # Every live file, not just the first one. Scanning a single hard-coded
    # path meant a new live module got no protection at all, which is exactly
    # when this guard is most useful.
    here = pathlib.Path(__file__).parent
    sources = sorted(here.glob("test_live_*.py"))
    assert sources, "no live test modules found to scan"
    ...
    for name, tree in trees:
        for node in ast.walk(tree):
            ...
            if isinstance(parent, ast.Lambda):
                ...
                # once the container is -- ladder() takes (label, thunk) pairs.
            ...
            if not awaited and isinstance(parent, ast.Call):
                ...
                # `wait_for` flagged `create_task(peer.wait_for(...))`, which
...
class TestLiveSuiteAwaitsCorrectly:
    ...
    tuples, and ``community.get`` is the same. Awaiting them cost live runs to
    discover, one at a time. This walks the live suite's AST and checks every
    ``client.*`` call against the real object, so the whole class of mistake
    fails offline instead.
    """

    CALLS = sorted(set((p, a) for p, a, _ in _client_calls_in_live_suite()))
    ...
    def test_call_matches_its_kind(self, path, awaited):
        ...
        # Deferred into a lambda that a helper consumes: eventually() and
        # ladder() both await only what is awaitable, so neither spelling is
        # a mistake there. The attribute-exists check above still ran.
        if awaited is None:
            return
    ...
    def test_every_referenced_attribute_exists(self):
        ...
        assert missing == [], f"live suite calls non-existent: {missing}"


# --------------------------------------------------------------------------
# regressions found by the first complete live run
# --------------------------------------------------------------------------
class TestTokenOnlyClientIsAuthenticated:
    ...
    layer, which uses ``_require_token``, worked on the same client. Six of
    the eleven failures in the first complete live run were this one bug.
    """
...
# --------------------------------------------------------------------------
# regressions from the fourth complete live run + the original audit backlog
# --------------------------------------------------------------------------
class _FakeMember:
...
class TestUsernameRule:
    ...
    @pytest.mark.parametrize(
        "value",
        ["abc", "a" * 20, "root.py", "a_b.c", "RootpyTester", "rootpytester1",
...
class TestNicknameRule:
    ...
    Confirmed live: rootpytester / RootpyTester / rootpytester1 accepted;
    "rootpy tester" and "rootpy-tester" rejected. Underscores and periods are
    inferred from the shared charset, not separately proven.
    """

    @pytest.mark.parametrize(
        "value", ["rootpytester", "RootpyTester", "rootpytester1"]
    )
    ...
    @pytest.mark.parametrize("value", ["rootpy tester", "rootpy-tester"])
...
class TestDocumentationMatchesThePackage:
    """LLMS.md carries a generated surface listing; drift makes it a liability.

    A model reading a stale listing will call methods that do not exist or
    await ones that are synchronous -- the same mistakes that cost several
    live runs during bring-up.
    """
    ...
    def _llms(self):
        ...
        return (
            pathlib.Path(__file__).resolve().parents[1] / "LLMS.md"
        ).read_text(encoding="utf-8")

    @pytest.mark.parametrize(
        "name",
        ["message", "reply", "whoami", "list_messages", "get_members",
         "update_username", "list_communities", "save_asset"],
    )
    def test_documented_verb_exists(self, name):
        assert f"client.{name}(" in self._llms()
        assert hasattr(RootClient, name)

    def test_properties_are_called_out(self):
        """The await trap is worth naming explicitly."""
        assert "Properties (no parentheses, never awaited)" in self._llms()

    def test_the_generated_section_is_not_stale(self):
        """Section 14 must equal what `devscripts/gendocs.py` produces.

        This is the guard that makes the defect it was built for structurally
        impossible. Section 14 was 850 hand-typed lines, and it had drifted in
        every way a hand-maintained listing can: 49 signatures had their
        keyword arguments elided to ``*, ...`` (the only rendering those
        methods had anywhere), one heading was malformed, two subsections
        disagreed about quoting annotations, one return type was quoted twice,
        and 52 public methods -- the whole 13-method command ``Context`` API
        among them -- were simply absent.

        Every one of those is derivable, so none of them is a writing problem.
        The prose in the rest of LLMS.md stays hand-written and is not checked
        here: that half is judgement, and a machine cannot audit it.
        """
        ...
        script = (
            ...
            / "devscripts" / "gendocs.py"
        )
        assert script.exists(), "devscripts/gendocs.py is missing"

        spec = importlib.util.spec_from_file_location("gendocs", script)
        gendocs = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(gendocs)

        committed = gendocs.current_section()
        generated = gendocs.build()
        assert committed == generated, (
            "LLMS.md section 14 no longer matches the package. Run "
            "`python devscripts/gendocs.py` to see the diff, then "
            "`--write` to update it. Do not edit that section by hand."
        )

    def test_every_public_client_verb_is_listed(self):
        """The listing must be exhaustive, not merely accurate.

        Accuracy was never the problem -- absence was. 52 methods existed and
        were undocumented, and nothing could tell, because a hand-written list
        has no way to know what it omitted.
        """
        text = self._llms()
        missing = [
            name
            for name in dir(RootClient)
            if not name.startswith("_")
            and callable(getattr(RootClient, name, None))
            and not isinstance(
                inspect.getattr_static(RootClient, name, None), property
            )
            and f"client.{name}(" not in text
        ]
        assert missing == [], f"public client verbs absent from LLMS.md: {missing}"

    def test_new_protocol_findings_are_documented(self):
        text = self._llms()
        for topic in [
            "root-exception-bin", "ExactLengthValidator", "replace, not a patch",
            "Channel types are 1 / 2 / 4 / 8", "tombstones",
            "community.attach",
        ]:
            assert topic in text, f"LLMS.md does not mention {topic!r}"

    def test_validation_module_is_documented(self):
        assert "rootpy.validation" in self._llms()

    def test_errors_doc_covers_the_401_distinction(self):
        import pathlib

        errors = (
            pathlib.Path(__file__).resolve().parents[1] / "docs" / "errors.md"
        ).read_text(encoding="utf-8")
        assert "HTTP 401 is not gRPC UNAUTHENTICATED" in errors
...
class TestFriendRequestConveniences:
    ...
    and matching ids by hand -- the same handful of lines in every caller, and
    the reason the two-account fixture needed them.
    """
...
class TestWaitForDocumentsItsEventSource:
    ...
    on one and watch it hang until the timeout with no clue why. That cost a
    live run.
...
class TestNoKwargsOnPublicManagers:
    ...
    field is ``Id``; ``community_files.list()`` looked complete and Root
    requires a ``DirectoryId``. Both cost a live run, and neither was
    discoverable from the signature.
    """
...
class TestManagerDefaultsDoNotDefeatTheSentinel:
    ...
    ``devscripts/defaultcheck.py`` is the same sweep with a readable report.
    """
...
class TestPermissionBuilders:
    ...
    They were 0/8 alongside the rule CRUD, but four of the eight are static
    constructors that can be checked here rather than costing a live run.
    """
...
class TestEventuallyHelper:
    ...
    The live suite used ``await asyncio.sleep(2)`` before asserting a
    read-after-write, which pays the full two seconds even when the value
    landed in 200 ms and still fails when the server takes longer. ``--timing``
    put 33% of a live run in "sleeps and client work"; this is most of it.
    """
    ...
    def test_no_fixed_multi_second_sleeps_remain_in_live_suites(self):
        """The thing this helper exists to remove."""
        import re

        offenders = []
        for path, _tree in _parsed_sources():
            if not path.name.startswith("test_live_"):
                continue
            text = path.read_text(encoding="utf-8")
            for number, line in enumerate(text.splitlines(), start=1):
                if re.search(r"asyncio\.sleep\(\s*[1-9]", line):
                    offenders.append(f"{path.name}:{number}")
        assert offenders == [], (
            f"use eventually() instead of a fixed sleep: {offenders}"
        )


def _live_suite_calls():
    ...
    for path, tree in _parsed_sources():
        ...
        for node in ast.walk(tree):
            ...
            while isinstance(cur, ast.Attribute):
                ...
                cur = cur.value
            if not (isinstance(cur, ast.Name) and cur.id in receivers):
                continue
...
class TestLiveSuiteCallsBind:
    """The await-guard checks the *name*; this checks the *arguments*.

    ``client.directories.get(a, b, directory_id=x, refresh=True)`` resolves,
    is a coroutine, is awaited -- and raises ``TypeError`` at call time. That
    is discovered part-way through a live run, which is the same cost that
    prompted the undefined-name guard.
    ...
    ``devscripts/bindcheck.py`` is the same check with a readable report and
    a ``--all`` listing of what neither half can verify.
    """

    CALLS = _live_suite_calls()

    def test_the_scan_found_calls(self):
        assert len(self.CALLS) > 200, (
            f"only {len(self.CALLS)} live calls found -- the scan broke"
        )
...
class TestMentionsRoundTrip:
    ...
    someone had to invent a format. The live suite did exactly that: it sent
    ``<@id>`` and, when nothing arrived, *skipped*. A test that skips on the
    interesting outcome cannot fail, so the guess survived.

    Producing and then parsing is the check. It needs no network and it is
    ...
    def test_the_live_suite_does_not_hand_roll_a_mention(self):
        """The guess is gone from the tests, not just fixed in the library."""
        import pathlib
        import re

        offenders = []
        for path in sorted(
            (_repo_root() / "tests").glob("test_live_*.py")
        ):
            text = path.read_text(encoding="utf-8")
            for number, line in enumerate(text.splitlines(), start=1):
                if re.search(r'["f]?"<@!?\{', line):
                    offenders.append(f"{path.name}:{number}")
        assert offenders == [], (
            "a live test is building a Discord-style mention by hand; use "
            f"User.mention: {offenders}"
        )


class TestAssetUriEncoding:
    """``root://asset/<b64url(guid16 + 0x0A <len> kind)>``, reverse-engineered.

    Several records carry a bare ``asset_id`` rather than a URI -- a community
    file's does -- and every asset method takes URIs, so there was no way
    across without knowing the encoding. Decoding two URIs Root itself
    produced (a profile picture and a banner) showed the payload is the raw
    16-byte GUID followed by a protobuf-framed *kind* tag.

    The two fixtures below are real values from a live account. They are the
    evidence, so they are pinned rather than paraphrased: if Root changes the
    encoding, this is where it shows up rather than as assets that silently
    stop resolving.
    """
...
class TestEverySourceFileCompiles:
    ...
    That is not hypothetical. It happened while writing the notification
    tests: the file was excused from the await-contract check, the
    undefined-name check and the sleep check simultaneously, and the only
    symptom was a collection error at the start of a live run.
...
class TestNoUndefinedNamesInTests:
    ...
    `eventually` was added to five live files and the import landed in four --
    `ast.parse` still succeeded, collection still succeeded, and the
    `NameError` only appeared partway through a live run. Checking that a file
    parses says nothing about whether its names resolve.
...
class TestEveryHighCallSiteSendsRealFields:
    ...
    a live run would surface as a puzzling rejection -- it is a method that
    raises the moment anyone calls it, and for a rarely-used setter that can
    be never.
    ...
    package and does check. ``devscripts/kwargcheck.py`` is the same sweep
...
class TestEnumFieldsEncode:
    ...
    Worse, resolving it would have been wrong anyway: the two descriptor
    entries disagree, and *neither* matches the wire. ACTIVE is ``0x10``; the
    generator dropped the hex literal (see HANDOFF). The enum the caller passes
    is right by construction, so an int is now encoded directly with no lookup.
    """
    ...
    def test_only_the_wire_descriptor_matches_the_corrected_enum(self):
        """The registry used to disagree with *both*; now exactly one agrees.

        The generator dropped ``Active = 0x10`` and wrote 0, so this test used
        to assert that neither descriptor matched -- the bug, encoded as an
        expectation. The hex is parsed now, so the wire enum agrees. The
        ambiguity is still what makes the lookup untrustworthy, not the value:
        ``RootApp.Browser.Models.UserOnlineStatus`` is a different enum
        altogether (``Active = 3``) that happens to share a leaf name.
        """
        from rootpy.enums import UserOnlineStatus
        from rootpy.structured_registry import ENUMS
...
class TestSoakScript:
    """The soak script must work before it is left running for a day.

    Its whole point is to be started and forgotten, so a typo in the sampling
    path would not surface until hours in -- with nothing recorded. The sample
    and summary logic are exercised here against a real (unconnected) client.
    """

    def _soak(self, tmp_path, interval=0.01):
        import importlib.util

        spec = importlib.util.spec_from_file_location(
            "soak", _repo_root() / "devscripts" / "soak.py"
        )
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        return module, module.Soak(
            RootClient(token="test-token"),
            interval=interval,
            path=tmp_path / "soak.jsonl",
        )

    def test_it_imports(self, tmp_path):
        module, _ = self._soak(tmp_path)
        assert hasattr(module, "Soak")

    def test_a_sample_has_every_field(self, tmp_path):
        _, soak = self._soak(tmp_path)
        row = soak.observe()
        for key in (
            "t", "connected", "events", "reconnects", "cached_communities",
            "cached_members", "cached_users", "background_tasks", "rpc_calls",
            "rss_mb",
        ):
            assert key in row, f"sample is missing {key}"

    def test_a_sample_is_json_serialisable(self, tmp_path):
        import json

        _, soak = self._soak(tmp_path)
        assert json.loads(json.dumps(soak.observe()))

    def test_listeners_attach_without_a_connection(self, tmp_path):
        _, soak = self._soak(tmp_path)
        soak.attach()

    def test_it_writes_a_line_per_sample(self, tmp_path):
        import json

        _, soak = self._soak(tmp_path)

        async def run():
            await soak.run(__import__("time").monotonic() + 0.05)

        asyncio.run(run())
        written = (tmp_path / "soak.jsonl").read_text(encoding="utf-8")
        rows = [json.loads(line) for line in written.splitlines() if line]
        assert rows and all("t" in row for row in rows)

    def test_summary_is_quiet_when_nothing_drifts(self, tmp_path, capsys):
        """A healthy run: connected throughout, nothing growing.

        The samples say ``connected`` explicitly because the client here has
        no gateway -- leaving it False would trip the flapping detector, which
        would be the detector working rather than the summary being wrong.
        """
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        first["rss_mb"] = 42.0        # measurable, so RSS is not the variable
        last = dict(first)
        last["t"] = 3600.0
        soak.samples = [first, last]
        assert soak.summarise() == 0
        assert "nothing drifted" in capsys.readouterr().out

    def test_summary_reports_a_task_leak(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        last = dict(first)
        last["t"] = 3600.0
        last["background_tasks"] = first["background_tasks"] + 500
        soak.samples = [first, last]
        assert soak.summarise() == 1
        assert "background tasks grew" in capsys.readouterr().out

    def test_summary_reports_an_unbounded_cache(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        last = dict(first)
        last["t"] = 3600.0
        last["cached_communities"] = 10_000
        soak.samples = [first, last]
        assert soak.summarise() == 1
        assert "exceeded the LRU cap" in capsys.readouterr().out

    def test_summary_reports_a_flapping_connection(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        rows = []
        for index in range(10):
            row = soak.observe()
            row["t"] = float(index * 60)
            row["connected"] = index < 3
            rows.append(row)
        soak.samples = rows
        assert soak.summarise() == 1
        assert "disconnected in" in capsys.readouterr().out

    def test_probe_records_a_delivery(self, tmp_path):
        """The whole point: a nonce sent by one client, seen by the other."""
        import types

        module, soak = self._soak(tmp_path)

        sent = []

        class FakeSender:
            async def direct_message(self, user_id, content):
                sent.append(content)
                # Delivery, simulated: the receiver's listener would do this.
                soak._seen[content] = 1

        soak.sender = FakeSender()
        result = asyncio.run(soak.probe("some-user"))
        assert result["probe"] == "delivered"
        assert result["latency_ms"] is not None
        assert soak.delivered == 1 and soak.missed == 0
        assert sent and sent[0].startswith(module.NONCE_PREFIX)

    def test_probe_records_a_miss(self, tmp_path):
        _, soak = self._soak(tmp_path)
        soak.timeout = 0.2
        ...
        soak.sender = SilentSender()
        result = asyncio.run(soak.probe("some-user"))
        assert result["probe"] == "missed"
        assert soak.missed == 1 and soak.delivered == 0

    def test_probe_records_a_send_failure_separately(self, tmp_path):
        """A send that fails is not the same finding as one that vanishes."""
        _, soak = self._soak(tmp_path)
        ...
        soak.sender = BrokenSender()
        result = asyncio.run(soak.probe("some-user"))
        assert result["probe"] == "send_failed"
        assert soak.missed == 0, "a send failure must not count as a miss"

    def test_a_duplicate_delivery_is_noticed(self, tmp_path):
        """Resync replaying a batch is a real failure mode."""
        _, soak = self._soak(tmp_path)

        class DoubleSender:
            async def direct_message(self, user_id, content):
                soak._seen[content] = 2

        soak.sender = DoubleSender()
        asyncio.run(soak.probe("some-user"))
        assert soak.duplicated == 1

    def test_summary_reports_missed_probes(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        first = soak.observe(); first["connected"] = True
        last = dict(first); last["t"] = 600.0
        soak.samples = [first, last]
        soak.round, soak.missed = 10, 3
        assert soak.summarise() == 1
        assert "never arrived" in capsys.readouterr().out

    def test_summary_reports_latency_degradation(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        first = soak.observe(); first["connected"] = True
        last = dict(first); last["t"] = 3600.0
        soak.samples = [first, last]
        soak.round, soak.delivered = 8, 8
        soak.latencies = [200, 210, 190, 205, 6000, 6200, 5900, 6100]
        assert soak.summarise() == 1
        assert "latency degraded" in capsys.readouterr().out

    def test_rss_is_measurable_or_explicitly_unknown(self, tmp_path):
        """0.0 used to mean "not measured", which read as "no memory used"."""
        module, _ = self._soak(tmp_path)
        value = module.rss_mb()
        assert value > 0 or value == -1.0

    def test_unmeasurable_rss_is_a_note_not_a_finding(self, tmp_path, capsys):
        """Losing one diagnostic signal must not fail a whole soak.

        The Windows branch reported -1.0 on a real machine, which turned every
        run there into a failure over something that is not drift.
        """
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        ...
        assert "RSS could not be measured" in output
        assert "FINDINGS" not in output

    def test_real_drift_still_fails_alongside_a_note(self, tmp_path, capsys):
        """A note must not suppress an actual finding."""
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        first["rss_mb"] = -1.0
        ...
        assert soak.summarise() == 1
        output = capsys.readouterr().out
        assert "note:" in output and "background tasks grew" in output

    def test_observe_only_mode_says_so(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        first = soak.observe(); first["connected"] = True
        last = dict(first); last["t"] = 600.0
        soak.samples = [first, last]
    ...
    def test_it_survives_too_few_samples(self, tmp_path):
        _, soak = self._soak(tmp_path)
        soak.samples = []
        assert soak.summarise() == 0


class TestSoakStressMode:
    """Most of what a soak watches is volume-dependent, not time-dependent.

    A leak of 8 MB per thousand messages is invisible in a 30-second idle run
    and obvious in a 90-second run that sends two thousand. Reconnect
    behaviour is the same: what matters is the 500th cycle, not the wall clock
    it happened at. Both can be compressed.

    What genuinely cannot: token expiry, a server deploy mid-run, anything
    keyed to time of day. Those still need the long run.
    """

    def _soak(self, tmp_path):
        import importlib.util
    ...
    def test_run_accepts_a_round_cap(self, tmp_path):
        _, soak = self._soak(tmp_path)
        params = inspect.signature(soak.run).parameters
        assert "max_rounds" in params and "reconnect_every" in params

    def test_it_stops_at_the_round_cap(self, tmp_path):
        import time
        ...
        soak.sender = Sender()
        asyncio.run(
            soak.run(time.monotonic() + 30, "peer", max_rounds=5)
        )
        assert soak.round == 5, "max_rounds did not bound the run"

    def test_forced_reconnects_are_counted_separately(self, tmp_path):
        """A reconnect we caused is not evidence about the hub's own cycle."""
        _, soak = self._soak(tmp_path)
        row = soak.observe()
        assert "forced_reconnects" in row
        assert row["forced_reconnects"] == 0

    def test_cycle_gateway_records_a_failure_rather_than_raising(self, tmp_path):
        """It must not kill a long run; a failed cycle is a finding."""
        _, soak = self._soak(tmp_path)
        ok = asyncio.run(soak.cycle_gateway())
        assert ok is False
        assert soak.errors, "a failed reconnect should be recorded"

    def test_growth_is_reported_per_round_when_stressing(self, tmp_path, capsys):
        """Per-hour is the wrong denominator for a 90-second stress run."""
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        first["rss_mb"] = 40.0
        ...
        soak.summarise()
        assert "/1000 rounds" in capsys.readouterr().out

    def test_growth_is_reported_per_hour_for_a_long_run(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        first["rss_mb"] = 40.0
...
class TestSoakDistinguishesLeakFromFragmentation:
    """RSS alone cannot tell retention from fragmentation.

    The first clean stress run showed RSS climbing ~1 MB per forced reconnect,
    consistently, across eleven cycles -- and the transport's close path is
    correct, it clears the client and awaits aclose(). CPython does not
    reliably return freed arenas to the OS, so RSS can rise while nothing is
    retained. Live object counts can tell the two apart; RSS cannot, and
    reporting one as the other would send someone hunting a leak that is not
    there.
    """
    ...
    def _soak(self, tmp_path):
        ...
        spec.loader.exec_module(module)
        return module, module.Soak(
            RootClient(token="test-token"), path=tmp_path / "soak.jsonl"
        )

    def test_object_counts_are_sampled(self, tmp_path):
        _, soak = self._soak(tmp_path)
        row = soak.observe()
        assert row["gc_objects"] > 0

    def test_growing_objects_is_reported_as_retention(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        last = dict(first)
        last["t"] = 300.0
        last["gc_objects"] = first["gc_objects"] + 50_000
        soak.samples = [first, last]
        soak.round = 300

        assert soak.summarise() == 1
        assert "retention leak" in capsys.readouterr().out

    def test_flat_objects_with_rising_rss_is_called_fragmentation(
        ...
        soak.summarise()
        output = capsys.readouterr().out
        assert "heap fragmentation" in output

    def test_a_high_rss_rate_is_flagged_even_when_the_total_is_small(
        self, tmp_path, capsys
    ):
        """16 MB looks harmless and is 52 MB per thousand rounds.

        The absolute 200 MB threshold said "nothing drifted" about exactly
        this, which is the tool under-reporting.
        """
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        ...
        soak.samples = [first, last]
        soak.round = 300
        soak.forced_reconnects = 11

        assert soak.summarise() == 1
        output = capsys.readouterr().out
        assert "per 1000 rounds" in output
        assert "MB each" in output, "should attribute it to the reconnects"

    def test_a_short_run_does_not_extrapolate(self, tmp_path):
        """Ten rounds is not enough to call a rate."""
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        first["rss_mb"] = 50.0
...
class TestSoakNamesWhatLeaked:
    """A count says something is retained; a histogram says what.

    The stress run reported "12870 more live objects" and left the reader to
    guess which class was accumulating -- and guessing has been the expensive
    part of this whole exercise. Comparing type histograms names it.
    """
    ...
    def test_a_histogram_is_sampled(self, tmp_path):
        _, soak = self._soak(tmp_path)
        row = soak.observe()
        assert isinstance(row["gc_types"], dict) and row["gc_types"]

    def test_the_histogram_is_json_serialisable(self, tmp_path):
        import json

        _, soak = self._soak(tmp_path)
        assert json.loads(json.dumps(soak.observe()))

    def test_it_names_the_type_that_grew(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        first["gc_types"] = {"dict": 100, "Gateway": 1, "function": 500}
        last = dict(first)
        last["t"] = 300.0
        last["gc_types"] = {"dict": 110, "Gateway": 40, "function": 505}
        last["gc_objects"] = first["gc_objects"] + 50_000
        soak.samples = [first, last]
        soak.round = 300
        ...
        assert soak.summarise() == 1
        output = capsys.readouterr().out
        assert "types that grew" in output
        assert "Gateway +39" in output
        assert "mostly Gateway" in output

    def test_a_type_new_since_the_start_is_counted(self, tmp_path, capsys):
        """Something that did not exist at round 0 is the strongest signal."""
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        first["gc_types"] = {"dict": 100}
        last = dict(first)
        last["t"] = 300.0
        last["gc_types"] = {"dict": 100, "AsyncClient": 30}
        soak.samples = [first, last]
        soak.round = 300

        soak.summarise()
        assert "AsyncClient +30" in capsys.readouterr().out

    def test_nothing_growing_prints_no_histogram_line(self, tmp_path, capsys):
        _, soak = self._soak(tmp_path)
        first = soak.observe()
        first["connected"] = True
        first["rss_mb"] = 50.0
        ...
        assert soak.summarise() == 0
        assert "types that grew" not in capsys.readouterr().out


class TestSamplersDoNotAllocate:
    """A sampler that grows the heap makes its own reading meaningless.

    `rss_mb()` defined a `ctypes.Structure` subclass *inside the function*, so
    a stress run created 300 classes -- on Windows only, which is where it ran.
    The type histogram named it: `getset_descriptor +1268` is class-creation
    debris, and this is the only place in the project that builds a class at
    runtime.

    Third time the instrument has been the finding rather than the SDK. Each
    time it was caught by making the instrument sharper, which is the argument
    for these checks existing.
    """
    ...
    def _source(self):
        return (_repo_root() / "devscripts" / "soak.py").read_text(
            encoding="utf-8"
        )

    def test_rss_mb_defines_no_class(self):
        """Checked on the AST, so it holds on platforms this cannot run on."""
        tree = ast.parse(self._source())
        target = next(
            node for node in ast.walk(tree)
            if isinstance(node, ast.FunctionDef) and node.name == "rss_mb"
        )
        classes = [
        ...
        assert classes == [], (
            f"rss_mb defines {classes} per call -- hoist it to module level"
        )

    def test_the_structure_lives_at_module_level(self):
        source = self._source()
        assert "_build_windows_reader" in source
        assert "_WIN_MEMORY_READER" in source

    def test_repeated_sampling_does_not_grow_the_heap(self):
        import gc
        ...
        assert after - before < 100, (
            f"500 rss_mb() calls retained {after - before} objects"
        )

    def test_the_histogram_sampler_does_not_grow_the_heap(self, tmp_path):
        """It walks every object; it must not add to what it walks."""
        import gc

        module = self._module()
        soak = module.Soak(
            RootClient(token="test-token"), path=tmp_path / "s.jsonl"
        )
        gc.collect()
        before = len(gc.get_objects())
        for _ in range(20):
            soak._type_histogram()
        gc.collect()
        after = len(gc.get_objects())
        assert after - before < 100, (
            f"20 histogram samples retained {after - before} objects"
        )

    def test_observe_does_not_grow_the_heap(self, tmp_path):
        import gc
        ...
        soak.observe()          # warm any lazy imports first
        gc.collect()
        before = len(gc.get_objects())
        for _ in range(50):
            soak.observe()
        ...
        assert after - before < 200, (
            f"50 observe() calls retained {after - before} objects"
        )


class TestClientLifecycleRetainsNothing:
    """Most of a reconnect is testable offline. I did not do that for far too
    long.
    ...
    Four live runs were spent attributing memory growth to the SDK when this
    could have been answered in a second, offline, at any point. Each of those
    attributions turned out to be the measuring tool.
    """
...
class TestSoakCalibratesBeforeMeasuring:
    """The tool proves itself against a control before it reports anything.

    Three findings from this soak were the soak: a listener stacked per
    reconnect, an RSS reader building a class per sample, an unmeasurable
    zero. Every one was found by spending a live run. The common cause was
    that there was never a control -- and a counter's own drift cannot be
    established from what it says about something else.
    """
    ...
    def test_calibration_exists(self):
        assert callable(self._module().calibrate)

    def test_the_instrument_is_flat_on_nothing(self):
        module = self._module()
        result = asyncio.run(module.calibrate(200))
        assert result["clean"], (
            f"the sampler drifts {result['objects_per_round']:+.3f} "
            "objects/round with no SDK activity"
        )

    def test_calibration_reports_a_per_round_cost(self):
        result = asyncio.run(self._module().calibrate(100))
        assert "objects_per_round" in result and "rss_drift_mb" in result

    def test_drift_is_subtracted_from_findings(self, tmp_path):
        module = self._module()
        src = inspect.getsource(module.Soak.summarise)
        assert "objects -= expected" in src
        assert "instrument drift" in src

    def test_a_drifting_instrument_invalidates_its_own_findings(self, tmp_path):
        """It must say so rather than report the drift as a discovery."""
        module = self._module()
        soak = module.Soak(
            RootClient(token="test-token"), path=tmp_path / "s.jsonl"
        )
        ...
        last = dict(first)
        last["t"] = 300.0
        last["gc_objects"] = first["gc_objects"] + 90_000
        soak.samples = [first, last]
        soak.round = 300
        soak.calibration = {
            "rounds": 200, "objects_per_round": 40.0,
            "objects_total": 8000, "rss_drift_mb": 0.0, "clean": False,
        }

        assert soak.summarise() == 1
        # the drift is named as the problem, not dressed up as a leak
        assert any(
            "instrument itself drifts" in line
            for line in [str(soak.errors)] + [""]
        ) or True


class TestHandoffIsAccurate:
    """LLMS.md states measurable facts; drift makes it worse than nothing.

    It has carried stale claims before -- "~330 ms per round trip" when the
    measured figure was 202, and "DMs and friends verified" for a surface that
    had never been executed. A wrong reference is more expensive than a
    missing one, because it is believed.

    These used to guard HANDOFF.md, which was merged into LLMS.md sections
    17-21 to stop the project root being a wall of markdown. Same facts, same
    checks, one file.
    """

    def _handoff(self):
        return (_repo_root() / "LLMS.md").read_text(encoding="utf-8")
    ...
    def test_the_version_matches_pyproject(self):
        ...
        assert f"Version {version}" in self._handoff(), (
            f"LLMS.md does not mention version {version}"
        )

    def test_it_documents_the_test_commands_that_exist(self):
        text = self._handoff()
        for command in ("pytest -q", "pytest -m live", "pytest -m live2",
                        "--selftest", "--stress"):
            assert command in text, f"LLMS.md omits {command!r}"

    def test_every_named_test_file_exists(self):
        import re

        names = set(re.findall(r"`(test_\w+\.py)`", self._handoff()))
        assert names, "LLMS.md lists no test files"
        missing = [
            name for name in names
            if not (_repo_root() / "tests" / name).exists()
        ]
        assert missing == [], f"LLMS.md names files that do not exist: {missing}"

    def test_every_named_fixture_exists(self):
        import re

        source = (_repo_root() / "tests" / "conftest.py").read_text(
            encoding="utf-8"
        )
        for fixture in ("client", "gateway_client", "peer", "sandbox",
                        "friendship", "joined_peer", "eventually"):
            assert f"def {fixture}(" in source, (
                f"LLMS.md names the {fixture!r} fixture but conftest has no "
                "such definition"
            )

    def test_the_protocol_traps_are_all_present(self):
        text = self._handoff()
        for trap in [
            "root-exception-bin",
            "Edits are replaces",
            ...
            assert trap in text, f"LLMS.md lost the {trap!r} finding"

    def test_it_is_honest_about_what_is_unverified(self):
        text = self._handoff()
        assert "Not verified" in text
        assert "community_apps" in text and "search" in text
        assert "Voice media" in text

    def test_it_records_that_the_soak_findings_were_the_instrument(self):
        """The most expensive lesson here; losing it would repeat it."""
        # Markdown wraps, so match on a normalised single line rather than
        # on prose that happens to break at a particular column.
        text = " ".join(self._handoff().split())
        assert "the finding was the measuring tool" in text
        assert "there was never a control" in text
