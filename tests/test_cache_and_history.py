...
class TestSocketResponseDebugTreesAreOptIn:
    ...
    def test_skipping_them_is_actually_cheaper(self):
        ...
        Best of three rather than one sample. A single timing run inside the
        full offline suite competes with whatever else the machine is doing,
        and one preempted sample was enough to fail this: it read 1.19x in a
        loaded run and 2.9x seconds later in isolation. Taking the minimum
        drops scheduler noise, which only ever inflates a measurement, without
        touching the 1.5x bound -- the claim being tested is unchanged.
        """
        import time
        ...
        def timed(**kwargs):
            return min(once(**kwargs) for _ in range(3))
...
class TestMessageListLimitWindow:
    ...
    It is tempting to read the rejections as "never send Limit at all", but
    ``history()`` sends ``Limit=page_size`` on every page and works. Root's
    own words settle it:

        Limit: Must be between 10 and 50 [InclusiveBetweenValidator]

    Omitted returns 50. Anything from 1 to 9 or above 50 is INVALID_ARGUMENT;
    10 to 50 are accepted and return exactly that many.
...
class TestHistoryWalksPastTheFirstPage:
    ...
    The cursor has to step by message id, not by time. A fake ``list`` that
    ignores the cursor, or ids that are not Root GUIDs, hides both halves of
    a faulty step -- which is why this test builds neither.
    """
