...
test is slow" but not "slow doing what". A live run is almost entirely network,
so the useful question is how much of each test is round-trip time, how much is
waiting on rate limits, and how much is client-side work -- and the SDK already
tracks exactly that in :class:`rootpy.stats.TransportStats`.

This records setup/call/teardown per test in milliseconds, reads the transport
counters around each one, and prints a summary. Session-scoped fixture cost
shows up in the setup column of whichever test triggered it, which is worth
...
def _transport_totals(item):
    ...
    Reading them off the live client rather than a global keeps the numbers
    honest when several clients are in play (the token-only one, the gateway
    one, and the peer each have their own transport).
    """
    totals = {"calls": 0, "roundtrip_ms": 0.0, "waiting_ms": 0.0,
              "overhead_ms": 0.0, "retries": 0, "rate_limited": 0}
    seen = set()
    for name in ("client", "gateway_client", "peer"):
